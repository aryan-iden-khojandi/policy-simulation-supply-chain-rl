...
class BraxGymnaxWrapper:
    def __init__(self, env_name, backend="positional", **kwargs):
...
def make_env(name, source, **kwargs):
    ctors = {
        "gymnax": make_env_gymnax,
        "brax": make_env_brax,
    }
    return ctors[source](name, **kwargs)


def make_env_gymnax(name, **kwargs):
    env, env_params = gymnax.make(name, **kwargs)
    env = FlattenObservationWrapper(env)
    env = LogWrapper(env)
    return env, env_params


def make_env_brax(name, normalize=False, gamma=0.99, **kwargs):
    env = BraxGymnaxWrapper(name, **kwargs)
    env = LogWrapper(env)
    env = ClipAction(env)
    if normalize:
