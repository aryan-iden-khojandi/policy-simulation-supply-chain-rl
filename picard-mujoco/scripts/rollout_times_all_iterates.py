...
def run_instance(
    ...
    return_error = jnp.sum(jnp.expand_dims(traj_B.reward, 0) - traj_Bp.reward, axis=1)
    return_ = traj_B.reward.sum()
    obs_error = jnp.sqrt(jnp.sum((jnp.expand_dims(traj_B.obs, 0) - traj_Bp.obs) ** 2, axis=2) .mean(axis=1))
    obs_diff = jnp.sqrt(
        jnp.sum((traj_B.obs - traj_A.obs) ** 2, axis=1).mean()
    )
    action_error = jnp.sqrt(
        jnp.sum(
            (jnp.expand_dims(traj_B.action, 0) - traj_Bp.action) ** 2,
            axis=2
        )
        .mean(axis=1)
    )
    ...
    metrics = {
        ...
        "iteration": jnp.arange(len(action_error)),
        "seed": seed
    }

    traj_Bp_final = jax.tree.map(op.itemgetter(-1), traj_Bp)
    return_error = jnp.sum(jnp.expand_dims(traj_Bp_final.reward, 0) - traj_Bp.reward, axis=1)
    return_ = traj_Bp_final.reward.sum()
    obs_error = jnp.sqrt(
        jnp.sum((jnp.expand_dims(traj_Bp_final.obs, 0) - traj_Bp.obs) ** 2, axis=2)
        .mean(axis=1)
    )
    obs_diff = jnp.sqrt(
        jnp.sum((traj_Bp_final.obs - traj_A.obs) ** 2, axis=1).mean()
    )
    action_error = jnp.sqrt(
        jnp.sum(
            (jnp.expand_dims(traj_Bp_final.action, 0) - traj_Bp.action) ** 2,
            axis=2
        )
        .mean(axis=1)
    )
    ...
    final_metrics = {
        "return_error_final": return_error,
        "return_final": return_,
        "obs_error_final": obs_error,
        "obs_diff_AB_final": obs_diff,
        "obs_norm_final": jnp.sqrt(jnp.sum(traj_B.obs ** 2, axis=1).mean()),
        "action_error_final": action_error,
        "action_norm_final": jnp.sqrt(jnp.sum(traj_B.action ** 2).mean()),
    }

    return pd.DataFrame(f.merge(metrics, final_metrics))
...
def main(
    ...
    print(jax.tree.map(lambda a, b: jnp.sqrt(((a - b) ** 2).mean()), params_A, params_B))
    ...
    picard_jit = jax.jit(
        partial(picard_rollout, env, env_params, policy, return_iterates=True, **picard)
    )
